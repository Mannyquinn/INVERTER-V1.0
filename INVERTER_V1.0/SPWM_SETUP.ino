float sineValues[59] =                                          //these are the values for a half cycle
 {   
...
void setupTimers()
{
  ...
  TCNT1 = 0;
  OCR1A = 100;
  OCR1B = 100;                                                   //This can be set between 0 and 511, but let the maximum be 500
...
{
  if(sineArrayIndex < 59 && positiveHalf == 1)
  {
      OCR1A = sineValues[sineArrayIndex] * multiplier;
      OCR1B = 0;
      sineArrayIndex++;
  }
  else if(sineArrayIndex < 59 && positiveHalf == -1)
  {
      OCR1B = sineValues[sineArrayIndex]* multiplier;
      OCR1A = 0;
      sineArrayIndex++;
  }
  else if(sineArrayIndex == 59 || sineArrayIndex > 59)
  {
