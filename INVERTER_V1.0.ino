...
#define SET_OUTPUT_VOLTAGE  10.0                                                                //5V
...
void loop() 
{
  ...
  error = map(error, 0, SET_OUTPUT_VOLTAGE,0 ,5);                                         //normalize error to 5, so the maximum error will always be 5 no matter the input voltage and the PID settings dont need to be changed for different voltages
  PIDControllerOutput = PID(error);                                               
 
  if(multiplier > 495 && outputVoltage < OVERLOAD_VOLTAGE)                                           //overload or shor 
   {
      runningError = true;
      shutDown();
   }
